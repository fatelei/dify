...
def _fetch_override_credentials(
    ...
    if credential_override.credential_id:
        ...
        # Try to get provider credential first
        try:
            credentials = provider_configuration.get_provider_credential(credential_override.credential_id)
            if isinstance(credentials, dict):
                return credentials
        except ValueError:
            explicit_not_found = True  # Provider explicitly signaled not found

        # Try to get model credential
        try:
            model_credential_result = provider_configuration.get_custom_model_credential(
                model_type=ModelType.LLM, model=model, credential_id=credential_override.credential_id
            )
            # Accept either a dict or an object with a 'credentials' mapping
            if isinstance(model_credential_result, dict):
                return model_credential_result.get("credentials", {})
            if hasattr(model_credential_result, "get"):
                creds = model_credential_result.get("credentials", {})
                if isinstance(creds, dict):
                    return creds
        except (ValueError, AttributeError):
            explicit_not_found = True

        # If we got explicit 'not found' errors from provider/model lookups, raise; otherwise return None
        if explicit_not_found:
            raise ValueError(f"Credential with ID {credential_override.credential_id} not found")
        return None
...
def fetch_model_config(
    ...
    # Check for workflow-level credential override
    ...
    # Override credentials if workflow-level override is provided
